  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          <h2 className="text-3xl font-extrabold text-primary-container tracking-tight">Gestión de Usuarios</h2>
          <p className="text-on-surface-variant font-medium mt-1">Administra los accesos al sistema</p>
        </div>
      ...
      <div className="bg-surface-container-lowest rounded-xl overflow-hidden border border-outline-variant/10 shadow-card">
            ...
            <tbody className="divide-y divide-outline-variant/5">
              {filtered.map((u, i) => (
                <tr key={u.id} className={`group hover:bg-surface-container-low/30 transition-colors ${i % 2 === 1 ? 'bg-surface-container-low/10' : ''}`}>
                  ...
                  <td className="px-6 py-5 text-sm text-on-surface-variant font-medium">{u.correo || u.email}</td>
                  <td className="px-6 py-5">
                    <Badge variant={rolVariant[u.rol]}>{rolLabels[u.rol] || u.rol}</Badge>
                  </td>
                  <td className="px-6 py-5">
                  ...
                  <td className="px-8 py-5 text-right">
                    <div className="flex items-center justify-end gap-2">
                      ...
                      <button onClick={() => handleToggle(u)} className={`p-2 rounded transition-all ${u.activo ? 'text-primary hover:bg-primary-fixed' : 'text-slate-300 hover:text-primary'}`}>
        ...
        <div className="p-6 bg-surface-container-low/50 flex justify-between items-center border-t border-outline-variant/10">
          <span className="text-sm font-medium text-on-surface-variant">
            ...
            <span className="text-primary">{users.length}</span> registros
          </span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {[
          { icon: 'verified_user', label: 'Seguridad', val: `${users.filter((u) => u.rol === 'ADMIN_RRHH' && u.activo).length} Usuarios Admin Activos` },
          { icon: 'history', label: 'Auditoría', val: 'Última actualización: ahora' },
          { icon: 'badge', label: 'Licencias', val: `${users.length} / 500 Cupos utilizados` },
        ].map((c) => (
